...
class JDWrapper(object):
    ...
    def good_stock(self, stock_id, good_count=1, area_id=None):
        ...
        payload = {
            'type': 'getstocks',
            'skuIds': str(stock_id),
            'area': area_id or '22_1930_50948_0',  # area change as needed
        }
    ...
    def buy(self, options):
        ...
        if options.qianggou:
            link = "http://cart.jd.com/gate.action?pid={0}&pcount=1&ptype=1".format(options.good)
        else:
        ...
        return False
    def getCartNum(self):
        link='https://cart.jd.com/cart/miniCartServiceNew.action?method=GetCart&callback=__jsonp1519786644060&_=1519786644060'
        resp = self.sess.get(link, cookies=self.cookies)
        return  resp
    def buy_good_count(self, good_id, count):
        ...
        payload = {
            ...
            'locationId': '22_1930_50948_0',  # need changed to your area location id
        }
...
if __name__ == '__main__':
    ...
    # for test
    if options.good == '':
        options.good = titan_xp
    options.submit = True
    options.flush = True
    options.qianggou=True
    '''
